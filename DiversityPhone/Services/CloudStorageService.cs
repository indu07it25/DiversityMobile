...
namespace DiversityPhone.Services
{
    ...
    public class CloudStorageService : ICloudStorageService
    {
        ...
        public async Task UploadFolderAsync(string LocalFolderPath, IProgress<int> PercentageProgress)
        {
            ...
            {
                if (!Iso.DirectoryExists(LocalFolderPath))
                {
                    throw new ArgumentException("Directory to be uploaded does not exist", "LocalFolderPath");
                }
        ...
        public async Task DownloadFolderAsync(string FolderName, string LocalTargetFolder, IProgress<int> PercentageProgress)
        {
            using (var Iso = IsolatedStorageFile.GetUserStoreForApplication())
            {
                if (!Iso.DirectoryExists(LocalTargetFolder))
                {
                    Iso.CreateDirectory(LocalTargetFolder);
                }

                var finalFolderName = Path.Combine(LocalTargetFolder, FolderName);
                if (Iso.DirectoryExists(finalFolderName))
                {
                    throw new ArgumentException(string.Format("Target folder already exists: {0}", finalFolderName), "FolderName");
                }
